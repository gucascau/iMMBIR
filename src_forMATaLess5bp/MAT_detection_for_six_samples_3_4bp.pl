...
use Getopt::Long;
my %opts;
GetOptions(\%opts,"i:s","m:s","n:s","o:s","p:s","q:s","f:s","g:s");
print "*************\n*$version*\n*************\n";
if ( !defined $opts{i} || !defined $opts{m} || !defined $opts{n}|| !defined $opts{o}|| !defined $opts{p} || !defined $opts{q}|| !defined $opts{f} || !defined $opts{g}) {
       	die "************************************************************************
       	Usage: $0
			-i: work directory
...
foreach my $i (keys %stringE){
	next unless ((($stringE{$i}=~/CTTCAGTATA/) || ($stringE{$i}=~/CTTCAGCATA/)) && ($stringE{$i}!~/TACTTCAGTATA/) && ($stringE{$i}!~/TACTTCAGCATA/));
...
foreach my $i (keys %stringF){
	next unless ((($stringF{$i}=~/CTTCAGTATA/) || ($stringF{$i}=~/CTTCAGCATA/)) && ($stringF{$i}!~/TACTTCAGTATA/) && ($stringF{$i}!~/TACTTCAGCATA/));
	my $inf6=$stringF{$i};
	$hash{$inf6}->{F}++;	
}
...
foreach my $n (keys %hash){
	...
	my $num=$numA+$numB+$numC+$numD+$numE+$numF;
	print OUT "$n\t$output\t$num\t$numA\t$numB\t$numC\t$numD\t$numE\t$numF\n";
}
