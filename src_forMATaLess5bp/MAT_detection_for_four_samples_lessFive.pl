...
foreach my $i (keys %stringA){
	next unless ((($stringA{$i}=~/TTCAGTATA/) || ($stringA{$i}=~/TTCAGCATA/)) && ($stringA{$i}!~/TACTTCAGTATA/) && ($stringA{$i}!~/TACTTCAGCATA/));
	my $inf1=$stringA{$i};
	$hash{$inf1}->{A}++;
...
foreach my $i (keys %stringB){
	next unless  ((($stringB{$i}=~/TTCAGTATA/) || ($stringB{$i}=~/TTCAGCATA/)) && ($stringB{$i}!~/TACTTCAGTATA/) && ($stringB{$i}!~/TACTTCAGCATA/));
	my $inf2=$stringB{$i};
	$hash{$inf2}->{B}++;	
}
...
foreach my $i (keys %stringC){
	next unless ((($stringC{$i}=~/TTCAGTATA/) || ($stringC{$i}=~/TTCAGCATA/)) && ($stringC{$i}!~/TACTTCAGTATA/) && ($stringC{$i}!~/TACTTCAGCATA/));
	my $inf3=$stringC{$i};
	$hash{$inf3}->{C}++;
...
foreach my $i (keys %stringD){
	next unless ((($stringD{$i}=~/TTCAGTATA/) || ($stringD{$i}=~/TTCAGCATA/)) && ($stringD{$i}!~/TACTTCAGTATA/) && ($stringD{$i}!~/TACTTCAGCATA/));
	my $inf4=$stringD{$i};
	$hash{$inf4}->{D}++;	
}
