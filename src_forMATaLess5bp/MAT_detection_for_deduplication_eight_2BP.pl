...
while (<I>){
	...
	### please pay attention that some cases the left Deletion might occur at the left MATa
		...
		if (($1+$3)>35 && ($1+$3)<45){
			...
			## this must not the insertion events we ignored 
			print "$_\n";
		}
	...
	### Left MAT side have two different deletions
		...
		$Isize{$i}=0;
		$seq{$i}=$_;
	}elsif($match=~m/^(\d+)M(\d+)D(\d+)M(\d+)D(\d+)M$/){
		if(($1+$3)>=45){
			my $part1=$1-5; 
			my $part2=$2; my $part3=$part1;
...
my %Tcov; my %MaxCov; my %MaxId;my %TcovA;my %TcovB;my %TcovC;my %TcovD; my %TcovE; my %TcovF; my %TcovG; my %TcovH;
...
foreach my $m (keys %partial){
	...
	my ($covA,$covB,$covC,$covD,$covE,$covF,$covG,$covH)=(split/:/,$id)[2,3,4,5,6,7,8,9];
...
print OUTF "ID\tStrainType\tInsertionFlank\tTotalCov\tCovA\tCovB\tCovC\tCovD\tCovE\tCovF\tCovG\tCovH\tMatchPosition\tInsSize\tSequence\n";
my $num=0;
foreach my $n (sort {$Tcov{$b} <=>$Tcov{$a}} keys %Tcov){
	...
	my $Ssize=$Isize{$name};
	print OUTF "$Sid\t$Stype\t$n\t$Tcov{$n}\t$TcovA{$n}\t$TcovB{$n}\t$TcovC{$n}\t$TcovD{$n}\t$TcovE{$n}\t$TcovF{$n}\t$TcovG{$n}\t$TcovH{$n}\t$IMAT\t$Ssize\t$ISeq\n";
	print OUT "$name\t$n\t$Tcov{$n}\t$TcovA{$n}\t$TcovB{$n}\t$TcovC{$n}\t$TcovD{$n}\t$TcovE{$n}\t$TcovF{$n}\t$TcovG{$n}\t$TcovH{$n}\t$Ssize\t$ISeq\t$seq{$name}\n";
}
