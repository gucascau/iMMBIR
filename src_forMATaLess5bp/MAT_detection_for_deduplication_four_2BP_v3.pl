...
foreach my $m (keys %partial){
	...
	my ($covA,$covB,$covC,$covD)=(split/:/,$id)[2,3,4,5];
...
foreach my $n (sort {$Tcov{$b} <=>$Tcov{$a}} keys %Tcov){
	...
	my $Ssize=$Isize{$name};
	print OUTF "$Sid\t$Stype\t$n\t$Tcov{$n}\t$TcovA{$n}\t$TcovB{$n}\t$TcovC{$n}\t$TcovD{$n}\t$IMAT\t$Ssize\t$ISeq\n";
	print OUT "$name\t$n\t$Tcov{$n}\t$TcovA{$n}\t$TcovB{$n}\t$TcovC{$n}\t$TcovD{$n}\t$Ssize\t$ISeq\t$seq{$name}\n";
}
