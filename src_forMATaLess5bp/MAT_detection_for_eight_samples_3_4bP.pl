...
foreach my $i (keys %stringG){
	next unless ((($stringG{$i}=~/CTTCAGTATA/) || ($stringG{$i}=~/CTTCAGCATA/)) && ($stringG{$i}!~/TACTTCAGTATA/) && ($stringG{$i}!~/TACTTCAGCATA/));
	my $inf7=$stringG{$i};
	$hash{$inf7}->{G}++;	
}

foreach my $i (keys %stringH){
	next unless ((($stringH{$i}=~/CTTCAGTATA/) || ($stringH{$i}=~/CTTCAGCATA/)) && ($stringH{$i}!~/TACTTCAGTATA/) && ($stringH{$i}!~/TACTTCAGCATA/));
	my $inf8=$stringH{$i};
	$hash{$inf8}->{H}++;	
}
