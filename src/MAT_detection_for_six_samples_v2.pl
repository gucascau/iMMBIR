...
foreach my $i (keys %stringD){
	next unless ($stringD{$i}=~/TACTTCAGTATA/|| $stringD{$i}=~/TACTTCAGCATA/);
	my $inf4=$stringD{$i};
	$hash{$inf4}->{D}++;	
}

foreach my $i (keys %stringE){
	next unless ($stringE{$i}=~/TACTTCAGTATA/|| $stringE{$i}=~/TACTTCAGCATA/);
	my $inf5=$stringE{$i};
	$hash{$inf5}->{E}++;
...
foreach my $i (keys %stringF){
	next unless ($stringF{$i}=~/TACTTCAGTATA/|| $stringF{$i}=~/TACTTCAGCATA/);
	my $inf6=$stringF{$i};
	$hash{$inf6}->{F}++;	
}
