...
my %Tcov; my %MaxCov; my %MaxId;my %TcovA;my %TcovB;my %TcovC;
...
foreach my $m (keys %partial){
	...
	my ($covA,$covB,$covC)=(split/:/,$id)[2,3,4];
...
print OUTF "ID\tStrainType\tInsertionFlank\tTotalCov\tCovA\tCovB\tCovC\tMatchPosition\tInsSize\tSequence\n";
my $num=0;
foreach my $n (sort {$Tcov{$b} <=>$Tcov{$a}} keys %Tcov){
	...
	my $Ssize=$Isize{$name};
	print OUTF "$Sid\t$Stype\t$n\t$Tcov{$n}\t$TcovA{$n}\t$TcovB{$n}\t$TcovC{$n}\t$IMAT\t$Ssize\t$ISeq\n";
	print OUT "$name\t$n\t$Tcov{$n}\t$TcovA{$n}\t$TcovB{$n}\t$TcovC{$n}\t$Ssize\t$ISeq\t$seq{$name}\n";
}
