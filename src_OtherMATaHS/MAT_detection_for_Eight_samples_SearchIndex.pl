...
use Getopt::Long;
my %opts;
GetOptions(\%opts,"i:s","m:s","n:s","o:s","p:s","q:s","r:s","f:s","a:s","b:s","c:s");
print "*************\n*$version*\n*************\n";
if ( !defined $opts{i} || !defined $opts{m} || !defined $opts{n}|| !defined $opts{o}|| !defined $opts{p} || !defined $opts{q} || !defined $opts{r} || !defined $opts{f}|| !defined $opts{a} || !defined $opts{b}|| !defined $opts{c}) {
       	die "************************************************************************
       	Usage: $0
			-i: work directory
...
my %stringH; my $id8;
open IN7, "$dir/$input8/Detection/$input8\_merged.assembled.fasta" or die $!;
while (<IN7>){
	chomp;
	if (/>(\S+)/){
		$id8=$1;
