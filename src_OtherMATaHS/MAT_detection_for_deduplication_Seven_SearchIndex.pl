...
my %Tcov; my %MaxCov; my %MaxId;my %TcovA;my %TcovB;my %TcovC;my %TcovD; my %TcovE; my %TcovF; my %TcovG; 
...
foreach my $m (keys %partial){
	...
	my ($covA,$covB,$covC,$covD,$covE,$covF,$covG)=(split/:/,$id)[2,3,4,5,6,7,8];
...
print OUTF "ID\tStrainType\tInsertionFlank\tTotalCov\tCovA\tCovB\tCovC\tCovD\tCovE\tCovF\tCovG\tMatchPosition\tInsSize\tSequence\n";
my $num=0;
foreach my $n (sort {$Tcov{$b} <=>$Tcov{$a}} keys %Tcov){
	...
	my $Ssize=$Isize{$name};
	print OUTF "$Sid\t$Stype\t$n\t$Tcov{$n}\t$TcovA{$n}\t$TcovB{$n}\t$TcovC{$n}\t$TcovD{$n}\t$TcovE{$n}\t$TcovF{$n}\t$TcovG{$n}\t$IMAT\t$Ssize\t$ISeq\n";
	print OUT "$name\t$n\t$Tcov{$n}\t$TcovA{$n}\t$TcovB{$n}\t$TcovC{$n}\t$TcovD{$n}\t$TcovE{$n}\t$TcovF{$n}\t$TcovG{$n}\t$Ssize\t$ISeq\t$seq{$name}\n";
}
